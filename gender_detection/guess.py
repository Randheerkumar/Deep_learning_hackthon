...
import math
import time
from gender_detection.data import inputs
import numpy as np
import tensorflow as tf
from gender_detection.model import select_model, get_checkpoint
from gender_detection.utils import *
import os
import json
import csv
...
def classify_many_single_crop(sess, label_list, softmax_output, coder, images, image_files, writer):
    try:
        num_batches = int(math.ceil(len(image_files) / MAX_BATCH_SZ))
...
def main(image_path,FLAGS):
    ...
    if FLAGS.face_detection_model:
        ...
        face_files, rectangles = face_detect.run(image_path)
        #print(face_files)
        files += face_files
        ...
        with tf.device(FLAGS.device_id):
            ...
            if len(files) == 0:
                if (os.path.isdir(image_path)):
                    for relpath in os.listdir(image_path):
                        abspath = os.path.join(image_path, relpath)
                    ...
                    if any([image_path.endswith('.' + ty) for ty in ('csv', 'tsv', 'txt')]):
                        files = list_images(image_path)
            ...
            if FLAGS.single_look:
                return classify_many_single_crop(sess, label_list, softmax_output, coder, images, image_files, writer)

            else:
                for image_file in image_files:
                    return classify_one_multi_crop(sess, label_list, softmax_output, coder, images, image_file, writer)
